"""suggested_answer cue: the anchor-phrase template pool + insertion logic.

Verbatim from the original cot-transparency codebase
...
def format_suggested_answer_bias(
    parsed_input: str,
    biased_answer: str,
    ...
    index_of_split = [i for i in range(len(question_lines)) if split_marker in question_lines[i]][0]

    possible_idxs = [0, index_of_split, len(question_lines), None]
    idx = random.Random(parsed_input).choice(possible_idxs)
    ...
    if add_instruction is not None:
        formatted_question = add_instruction(formatted_question)
...
def format_anchor_bias_question(
    ...
    return format_suggested_answer_bias(
        ...
        add_instruction=verbalize_func,
        split_marker=question.data_format.join_variant.value.strip("\n"),
    )
